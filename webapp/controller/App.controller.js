sap.ui.define([
	...
	return BaseController.extend("sap.support.boost.controller.App", {
		...
		handleDatePickerChange: function () {
			var oEventBus = sap.ui.getCore().getEventBus();
			var oselectedRegion = this.getModel("selectedKey").getProperty("/region");
			var oSelectedDate = this.getModel("datePick").getProperty("/date");
			var oCurrentdates = this.formatter.fnParseDate(new Date());

			if (this.getModel("selectedKey").getProperty("/page") === "day") {
				if (this.formatter.fnParseDate(oSelectedDate) >= "20240101") {
					if (oselectedRegion === "EMEA BACKOFFICE") {
						this.sRegionParam = this.getUrlVars()["region"] || "EMEA MEE BACKOFFICE";
						this.getModel("selectedKey").setProperty("/region", "EMEA MEE BACKOFFICE");
						this.getRegions();
						oEventBus.publish("app", "getDayStaffingDate");
						if (this.getModel("Teams").getData().length <= 0) {
							this.getTeams();
						}
						sap.m.MessageToast.show("EMEA BACKOFFICE" + "\n " + "Region is Auto redirecting to" + "\n " + "EMEA MEE BACKOFFICE.", {
							"at": "CenterCenter",
							"duration": "5000",
							"width": "80em"
						});
					} else if (oselectedRegion === "EMEA CCT") {
						this.sRegionParam = this.getUrlVars()["region"] || "EMEA MEE CCT";
						this.getModel("selectedKey").setProperty("/region", "EMEA MEE CCT");
						this.getRegions();
						oEventBus.publish("app", "getDayStaffingDate");
						if (this.getModel("Teams").getData().length <= 0) {
							this.getTeams();
						}
						sap.m.MessageToast.show("EMEA CCT" + "\n " + "Region is Auto redirecting to" + "\n " + "EMEA MEE CCT", {
							"at": "CenterCenter",
							"duration": "5000",
							"width": "80em"
						});
					} else {
						this.sRegionParam = this.getModel("selectedKey").getProperty("/region");
						this.getRegions();
						oEventBus.publish("app", "getDayStaffingDate");
						if (this.getModel("Teams").getData().length <= 0) {
							this.getTeams();
				...
				} else if (this.formatter.fnParseDate(oSelectedDate) <= "20231231") {
					if (oselectedRegion === "EMEA MEE BACKOFFICE") {
						this.sRegionParam = this.getUrlVars()["region"] || "EMEA BACKOFFICE";
						this.getModel("selectedKey").setProperty("/region", "EMEA BACKOFFICE");
						this.getRegions();
						oEventBus.publish("app", "getDayStaffingDate");
						if (this.getModel("Teams").getData().length <= 0) {
						...
						sap.m.MessageToast.show("EMEA MEE BACKOFFICE" + " " + "Region is Auto redirecting to" + " " + "EMEA BACKOFFICE", {
							"at": "CenterCenter",
							"duration": "3000"
						});
					} else if (oselectedRegion === "EMEA MEE CCT") {
						this.sRegionParam = this.getUrlVars()["region"] || "EMEA CCT";
						this.getModel("selectedKey").setProperty("/region", "EMEA CCT");
						this.getRegions();
						oEventBus.publish("app", "getDayStaffingDate");
						if (this.getModel("Teams").getData().length <= 0) {
							this.getTeams();
						}
						sap.m.MessageToast.show("EMEA MEE CCT" + " " + "Region is Auto redirecting to" + " " + "EMEA CCT", {
							"at": "CenterCenter",
							"duration": "3000"
						});
					} else {
			...
			if (this.getModel("selectedKey").getProperty("/page") === "dutyManager") {
			/*	this.getRegions();
				this.getTeams();
				oEventBus.publish("app", "getDTMNStaffingDate");*/
				if (this.formatter.fnParseDate(oSelectedDate) >= "20240101") {
					if (oselectedRegion === "EMEA BACKOFFICE") {
						this.sRegionParam = this.getUrlVars()["region"] || "EMEA MEE BACKOFFICE";
						this.getModel("selectedKey").setProperty("/region", "EMEA MEE BACKOFFICE");
		...
		openITDticketpress: function (oEvent) {
			...
			window.open(
				"https://fiorilaunchpad.sap.com/sites#Help-Inbox&/create/ZINE/IMAS_SUD_MC/Issue%20with%20'BOOST'/03/null/null/Issue%20Description%20and%20steps%20to%20reproduce%20this%20issue"
			);
		},
			...
			jQuery.ajax({
				...
				success: function (result, response, request) {
					var downTimestatus = result.SystemStatusNew.filter(function (item) {
						return (item.SYSTEMID === "ISP" || item.SYSTEMID === "IST" || item.SYSTEMID === "ISD" || item.SYSTEMID === "ICP" || item.SYSTEMID ===
							"ICT" || item.SYSTEMID === "ICD");
					});
					...
					for (var i = 0; i < downTimestatus.length; i++) {
						if (downTimestatus[i].AVAILABILITYSTATUS === "1") {
							...
							downTimestatus[i].Desc = "UnPlanned DownTime since" + " " + downTimestatus[i].LASTUNPLANDTSTART.split("T")[0] + " " +
								downTimestatus[i].LASTUNPLANDTSTART.split("T")[1].split("Z")[0] + "(CET/CEST)";
						}
						if (downTimestatus[i].AVAILABILITYSTATUS === "2") {
							downTimestatus[i].AVAILABILITYSTATUS = "Warning";
							downTimestatus[i].Desc = "Planned Downtime Until" + " " + downTimestatus[i].NEXTPLANDTEND.split("T")[0] + " " +
								downTimestatus[i].NEXTPLANDTEND.split("T")[1].split("Z")[0] + "(CET/CEST)";
						}
								...
								items: {
									...
									template: new sap.m.StandardListItem({
										...
										press: function (oEvent) {
											...
											var nxtUnplannedstart = oSelectobjData.NEXTPLANDTSTART === "" ? "No Data Available" : oSelectobjData.NEXTPLANDTSTART
												.split("T")[0] + " " + oSelectobjData.NEXTPLANDTSTART.split("T")[1].split("Z")[0] + "(CET/CEST)";
											var nxtUnplannedend = oSelectobjData.NEXTPLANDTEND === "" ? "No Data Available" : oSelectobjData.NEXTPLANDTEND.split(
												"T")[0] + " " + oSelectobjData.NEXTPLANDTEND.split("T")[1].split("Z")[0] + "(CET/CEST)";
											var lstUnplannedstart = oSelectobjData.LASTUNPLANDTSTART === "" ? "No Data Available" : oSelectobjData.LASTUNPLANDTSTART
												.split("T")[0] + " " + oSelectobjData.LASTUNPLANDTSTART.split("T")[1].split("Z")[0] + "(CET/CEST)";
											var lstUnplannedend = oSelectobjData.LASTUNPLANDTEND === "" ? "No Data Available" : oSelectobjData.LASTUNPLANDTEND
												.split("T")[0] + " " + oSelectobjData.LASTUNPLANDTEND.split("T")[1].split("Z")[0] + "(CET/CEST)";
											new sap.m.MessageBox.information("SystemID:" + " " + oSelectobjData.SYSTEMID +
												...
												"\n Last UnplannedEnd:" + " " + lstUnplannedend, {
													title: oSelectobjData.SYSTEMID + " " + "Downtime Status"
												});
										}
									})
								}
					...
					if (authCheck) {
						if (authCheck.Authorized) {
							jQuery.sap.delayedCall(500, this, function () {
								this.oDefaultDialog.getContent()[0].getBinding('items').sort(new sap.ui.model.Sorter({
									path: "SYSTEMID",
									descending: false
								}));
